---
title: "Predicting Raw COMPAS scores with Arrests"
author: "Beau Coker"
date: "6/27/2018"
output: html_document
---

This notebook contains the additional COMPAS general score analysis (with arrests instead of charges) that can be found in the rejoinder. 
...
```{r}
ggsave("Figures/predict_scores_general_arrests/age_LB_general.pdf", plot = age_spline_general, width = 3.5, height = 2.5, units = "in")
ggsave("Figures/predict_scores_general_arrests/age_LB_alldata_general.pdf", plot = age_spline_general_all, width = 3.5, height = 2.5, units = "in")
```
...
```{r}
ggsave("Figures/predict_scores_general_arrests/prior_arrests_LB_general.pdf", plot = prior_arrests_LB_general, width = 3.5, height = 2.5, units = "in")
```
...
```{r}
ggsave("Figures/predict_scores_general_arrests/crim_inv_LB_general_arrests.pdf",width = 3.5, height = 2.5, units = "in")
```
...
features %>%
  ...
  ggplot() +
    ...
    theme(text = element_text(size=9),
          ...
          legend.position="top")+
  ggtitle('Using Arrests')
```

```{r}
ggsave("Figures/predict_scores_general_arrests/crim_inv_LB_general_arrests_no_age_outliers.pdf",width = 3.5, height = 2.5, units = "in")
```
...
ggplot() +
    ...
    theme(text = element_text(size=9),
          axis.text=element_text(size=12),
          legend.position="top")+
  ggtitle(paste0('USING ARRESTS cutoff = ', outlier_cutoff))
```
...
p <- features %>%
  ...
  ggsave(paste0("Figures/predict_scores_general_arrests/crim_inv_weightings/",title,".png"),plot=p,width = 3.5, height = 2.5, units = "in")
}
```
...
ggplot()+
  ...
  xlim(xmin_crim_inv,xmax_crim_inv)+
  ylim(-0.5, 4) + 
  xlab("Criminal Involvement Subscale Sum -- Unfiltered ") +
  ylab(expression(General~score~-~f[age]))  +
  theme(text = element_text(size=18),
        axis.text=element_text(size=18),
        legend.position="none")
...
```{r}
ggsave("Figures/predict_scores_general_arrests/crim_inv_spline_general.pdf", plot = crim_inv_spline_general, width = 3.5, height = 2.5, units = "in")
ggsave("Figures/predict_scores_general_arrests/crim_inv_spline_alldata_general.pdf", plot = crim_inv_spline_general_all, width = 3.5, height = 2.5, units = "in")

```


## Check if any further dependence on age at first arrest
```{r}
age_first_offense_fage_gcriminv = ggplot()+
  
  geom_point(aes(x=p_age_first_offense, raw_score__age_spline__crim_inv_spline), # age spline outliers
             color="green", 
             data=features %>% filter(filt1, filt3, filt5,!filt8)) + 
  geom_point(aes(x=p_age_first_offense, raw_score__age_spline__crim_inv_spline), # crim inv poly outliers
             color="red", alpha=.3, 
             data=features %>% filter(filt1, filt3, filt5,filt8, !filt9)) + 
  geom_point(aes(x=p_age_first_offense, raw_score__age_spline__crim_inv_spline), # not outliers
             color="#619CFF", alpha=.3, 
             data=features %>% filter(filt1, filt3, filt5,filt8, filt9)) + 
        ...
        ylab(expression(General~score~-~f[age]~-~g[crim-inv]))  +
        theme(text = element_text(size=18),
        axis.text=element_text(size=18),
        legend.position="none")
age_first_offense_fage_gcriminv
```

```{r}
ggsave("Figures/predict_scores_general_arrests/age_first_offense_fage_gcriminv.pdf", plot = age_first_offense_fage_gcriminv, width = 6.5, height = 4, units = "in")
```
...
```{r}
ggsave("Figures/predict_scores_general_arrests/raw-fage_xgboost_gp3_general.pdf",width = 3, height = 3, units = "in")
```
...
```{r}
ggsave("Figures/predict_scores_general_arrests/raw-fage_xgboost_gp4_general.pdf",width = 3, height = 3, units = "in")
```
...
```{r}
ggsave("Figures/predict_scores_general_arrests/xgboost_rawScore_annotate_general.pdf",width = 4, height = 4, units = "in")
```
...
There are a few groups of predictors we will use:
* Group 6: using criminal history (only age variables)
* Group 7: without age variables (only criminal history)
* Group 8: with everything (age and criminal history)
...
```{r}
ggsave("Figures/predict_scores_general_arrests/raw_xgboost_gp3_general.pdf",width = 3, height = 3, units = "in")
```
