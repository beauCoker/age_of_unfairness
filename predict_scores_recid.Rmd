...
arrests = TRUE # use arrests to compute crim inv subscale
...
features = features %>%
  ...
  mutate(
    ...
    # Subscales:
    crim_inv = ifelse(rep(arrests, nrow(features)), #bool
                      p_arrest+ 
                      p_jail30+
                      p_prison+
                      p_probation,
...
### Look at COMPAS age remainder vs number of prior charges/arrests individually (both are important items in the crim inv subscale)
```{r}
...
### Look at COMPAS age remainder vs prior arrests 
...
```{r}
ggsave("Figures/prior_charges_LB_general.pdf",width = 3.5, height = 2.5, units = "in")
...
if (arrests == TRUE){
  ...
  ggsave("Figures/crim_inv_LB_general_charges.pdf",width = 3.5, height = 2.5, units = "in")
