...
```{r}
test = features %>% filter(crim_inv == 20, decile_score == 2)
View(test)
```
...
Also look at just number of prior charges
...
## Look at Compas remainder vs age at first arrest, given that everybody has 1 prior

```{r}
features %>%
  filter(filt1, filt3, p_charge == 1) %>% 
  select(p_age_first_offense, raw_score__age_spline, filt8) %>%
  ggplot() +
  geom_point(aes(x=p_age_first_offense,y=raw_score__age_spline,color=filt8),alpha=.5) +
  xlim(c(0,70))+
  theme_bw()+
  xlab("Age at first offense given p_charges=1") +
  ylab(expression(General~score~-~f[age]))  +
  theme(text = element_text(size=12),
        axis.text=element_text(size=12),
        legend.position="top") +
...
features %>%
  filter(filt1, filt3, p_charge_violent == 1) %>% 
  select(p_age_first_offense, raw_score__age_spline, filt8) %>%
  ggplot() +
  geom_point(aes(x=p_age_first_offense,y=raw_score__age_spline,color=filt8),alpha=.5) +
  xlim(c(0,70))+
  theme_bw()+
  xlab("Age at first offense given p_charges_violent=1") +
  ylab(expression(General~score~-~f[age]))  +
  theme(text = element_text(size=12),
        axis.text=element_text(size=12),
        legend.position="top") +
