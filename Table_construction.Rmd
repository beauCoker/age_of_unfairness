...
# Check dyn_arrest, dyn_charge  
REMOVE
```{r}
charges = dyn_charge %>% filter(person_id == 4) 

arrests = dyn_arrest %>% filter(person_id == 4)

charge_df = charges$charge %>% as.data.frame()
arrest_df = arrests$arrest %>% as.data.frame()

charge_df
arrest_df
#length(unique(res_df$arrest_date))
#res_df
```
